...
Checkpoint local: `1fd6205`, ledger `d3984ee`, web/eval `a267761` trên `claude/lab-eval-checkpoint`; chưa push.
...
- GĐ4 core: hoàn tất 30 case, ground truth review kỹ thuật, rubric, runner và baseline RAG/no-RAG.
...
- `eval/cases/*.json`: 30 alert `sanitized-live`.
- `eval/expected/*.json`: ground truth nháp.
- `eval/manifest.json`: case registry.
- `eval/rubric.md`: chấm 5 tiêu chí, thang 1–5.
...
- `tests/test_eval_dataset.py`: corpus/sanitization/reviewer-invariant tests.

Corpus sau review:

- 30 case: SSH 12, FIM 3, benign 13, ambiguous 2.
- 16 rule ID; thêm 6 benign case `23502` CVE solved.
...
| Chỉ số | RAG | No-RAG |
|---|---:|---:|
| Hoàn tất | 30/30 | 30/30 |
| Schema valid | 100% | 100% |
| Lỗi | 0 | 0 |
| Mean latency | 2.702s | 2.399s |
| Median latency | 2.563s | 2.435s |
| p95 latency | 3.440s | 2.925s |
| Severity exact-match draft | 18/30 | 21/30 |

Không kết luận RAG kém chỉ từ một lượt stochastic; cần human semantic scoring.

## Verification gần nhất

```text
python -m compileall -q ai_module eval tests: PASS
PYTHONPATH=ai_module python -m pytest tests -q: 20 passed
...
1. Reviewer người thứ hai chấm/adjudicate `eval/expected/*.json` và output hai CSV.
2. Đo baseline phân tích tay trên cùng 30 case theo `eval/README.md`.
3. Nếu cần model đối chứng, chạy cùng corpus/prompt/config; model local có sẵn `CyberCrew/notmythos-8b`.
4. Web cases `31101/31151` chưa có vì snapshot Indexer không chứa; chỉ thêm khi capture live, không synthetic để đủ quota.
5. Re-test SSH script mới trên Kali.
6. Cố tạo FIM rule `550` modified bằng dedicated marker, không đụng sensitive files.
7. Chạy `git config core.hooksPath .githooks` nếu muốn bật ledger gate cho clone hiện tại.
8. Review/stage đúng file trước commit; không stage `.claude/`, `.bak` hoặc `create_rag_data.py` mù.
